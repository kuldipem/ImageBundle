...
namespace Kem\ImageBundle\Controller;
...
use Kem\ImageBundle\Handler\UploadHandler;

class UploadController extends Controller
{
    /**
     * Save uploaded image according to kem_image field configuration
     *
    ...
    public function uploadImageAction(
        ...
        $thumbsDir = $this->container->getParameter('kem_image.thumbs_dir');
        $thumbSize = $this->container->getParameter('kem_image.media_lib_thumb_size');

        $galleryDir = $this->container->getParameter('kem_image.gallery_dir');
        $gThumbSize = $this->container->getParameter('kem_image.gallery_thumb_size');
        ...
        $transDomain = $this->container->getParameter('kem_image.translation_domain');
    ...
    public function cropImageAction(
        ...
        if (!is_dir($uploadUrl.'/'.$this->container->getParameter('kem_image.cropped_image_dir').'/')) {
            mkdir($uploadUrl.'/'.$this->container->getParameter('kem_image.cropped_image_dir').'/', 0755, true);
        }
        $ext = pathinfo($imageName, PATHINFO_EXTENSION);
        $imageName = sha1(uniqid(mt_rand(), true)).'.'.$ext;
        $destSrc = $uploadUrl.'/'.$this->container->getParameter('kem_image.cropped_image_dir').'/'.$imageName;
        ...
        $previewSrc = '/'.$config['uploadConfig']['webDir'].'/'.$this->container->getParameter('kem_image.cropped_image_dir').'/'.$imageName;
        if (isset($config['cropConfig']['thumbs']) && ($thumbs = $config['cropConfig']['thumbs']) && count($thumbs)) {
            $thumbDir = $uploadUrl.'/'.$this->container->getParameter('kem_image.cropped_image_dir').'/'.$this->container->getParameter('kem_image.thumbs_dir').'/';
            if (!is_dir($thumbDir)) {
                mkdir($thumbDir);
            }
            ...
            foreach ($thumbs as $thumb) {
                ...
                if (isset($thumb['useAsFieldImage']) && $thumb['useAsFieldImage']) {
                    $previewSrc = '/'.$config['uploadConfig']['webDir'].'/'.$this->container->getParameter('kem_image.cropped_image_dir').'/'.$this->container->getParameter('kem_image.thumbs_dir').'/'.$thumbName;
                }
            }
        }

        return new Response(json_encode(array('success' => true,
            'filename'=>$this->container->getParameter('kem_image.cropped_image_dir').'/'.$imageName,
            'previewSrc' => $previewSrc,
            'galleryThumb' => $this->container->getParameter('kem_image.cropped_image_dir').'/'.$this->container->getParameter('kem_image.thumbs_dir').'/'.$gThumbSize.'x'.$gThumbSize.'-'.$imageName)));
    }
    ...
    public function getLibraryImagesAction(Request $request)
    {
        ...
        $result['thumbsDir'] = $this->container->getParameter('kem_image.thumbs_dir');
